...
└── service/                # 비즈니스 로직
    ...
    ├── OrderServiceStrategy.java  # 주문 서비스 전략
    ├── ConcurrentOrderService.java # 동시성 주문 서비스
    ├── OptimisticOrderService.java # 낙관적 락 주문 서비스
...
### 3. 동시성 처리 전략
- **비관적 락 (Pessimistic Locking)**: `@Lock(LockModeType.PESSIMISTIC_WRITE)`
- **낙관적 락 (Optimistic Locking)**: `@Version` + `@Retryable`
- **동시성 락 (Concurrent Locking)**: `ReentrantLock` 기반 세밀한 제어
...
### 1. 프로젝트 클론
```bash
git clone https://github.com/kaos1025/HOMEWORK.git
cd HOMEWORK
```
...
```bash
# 경계값 테스트
./gradlew test --tests "*ShippingCalculatorBoundaryTest*"

# 동시성 테스트
./gradlew test --tests "*OrderServiceAdvancedConcurrencyTest*"

# 계약 테스트
./gradlew test --tests "*OrderApiContractTest*"
```
...
# 주문 처리 전략
order:
  strategy: optimistic  # pessimistic, concurrent, optimistic
